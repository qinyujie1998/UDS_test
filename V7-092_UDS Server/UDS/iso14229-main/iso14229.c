...
static uint8_t _0x34_RequestDownload(UDSServer_t *self) {
    ...
    if (self->xferIsActive) { //传输是否在进行中，没在传输中才可以开启
    ...
    err = decodeAddressAndLength(self, &self->recv_buf[2], &memoryAddress, &memorySize); //获取要下载的地址和大小
    if (kPositiveResponse != err) {
        return NegativeResponse(self, err);
    }
    ...
    ResetTransfer(self);
    self->xferIsActive = true; //传输开始进行
    self->xferTotalBytes = memorySize;//下载的数据大小
    self->xferBlockLength = args.maxNumberOfBlockLength;//每轮传输最大字节数
...
static uint8_t _0x35_RequestUpload(UDSServer_t *self) {
    ...
    err = decodeAddressAndLength(self, &self->recv_buf[2], &memoryAddress, &memorySize); //获取要下载的地址和下载的大小
    if (kPositiveResponse != err) {
        return NegativeResponse(self, err);
    }
...
static uint8_t _0x36_TransferData(UDSServer_t *self) {
    ...
    if (self->xferByteCounter + request_data_len > self->xferTotalBytes) { //没有超过总大小  xferTotalBytes == memorySize
...

    {
        UDSTransferDataArgs_t args = {
            ...
            .maxRespLen = self->xferBlockLength - UDS_0X36_RESP_BASE_LEN, //每轮传输最大字节数 xferBlockLength = maxNumberOfBlockLength
...
static uint8_t evaluateServiceResponse(UDSServer_t *self, const uint8_t addressingScheme) {
    ...
    if ((kTpAddrTypeFunctional == addressingScheme) &&
        ...
        if (suppressResponse) { /* Suppress positive re	sponse message */
            NoResponse(self);
...
UDSErr_t UDSServerInit(UDSServer_t *self, const UDSServerConfig_t *cfg) {
    ...
    self->p2_ms = 1;//UDS_SERVER_DEFAULT_P2_MS;      // 客户端发请求后，服务器端处理请求的溢出时间
    self->p2_star_ms = UDS_SERVER_DEFAULT_P2_STAR_MS;// 回复否定响应0x78后，在这个参数设置的时间内依然无法回复请求，时间溢出后要再次发送0x78
    self->s3_ms = UDS_SERVER_DEFAULT_S3_MS;          // 离开非默认状态时间
    self->fn = cfg->fn;
    ...
    self->p2_timer = UDSMillis() - self->p2_ms; //客户端发请求后，服务器端处理请求的溢出时间

    // Set the session timeout for s3 milliseconds from now.
    self->s3_session_timeout_timer = UDSMillis() + self->s3_ms;//设置会话溢出时间是3秒
...
void UDSServerPoll(UDSServer_t *self) {
	...
	// p2_timer p2_ms客户端发请求后，服务器端处理请求的溢出时间
    int size = 0;
    UDSTpAddr_t ta_type = kTpAddrTypePhysical;
    if (UDSTimeAfter(UDSMillis(), self->p2_timer)) { //50ms处理1次
...
static void PollLowLevel(UDSClient_t *client) {
    ...
    case kRequestStateAwaitSendComplete: {
        ...
        if (tp_status & UDS_TP_SEND_IN_PROGRESS) { //抑制回复
            ; // await send complete
        } else {
