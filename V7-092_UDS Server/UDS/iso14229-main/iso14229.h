...
//_Static_assert(UDS_CLIENT_DEFAULT_P2_STAR_MS > UDS_CLIENT_DEFAULT_P2_MS, ""); --注释掉了
...
#define UDS_SERVER_DEFAULT_P2_MS (50)  //客户端发请求后，服务器端处理请求的溢出时间
#endif

#ifndef UDS_SERVER_DEFAULT_P2_STAR_MS
#define UDS_SERVER_DEFAULT_P2_STAR_MS (2000) //回复否定响应0x78后，在这个参数设置的时间内依然无法回复请求，时间溢出后要再次发送0x78
#endif

#ifndef UDS_SERVER_DEFAULT_S3_MS
#define UDS_SERVER_DEFAULT_S3_MS (3000) //离开非默认状态时间
#endif

//P4server 接收到请求后，到final response的时间，如果P4server = P2Server_max，那么将禁止发送0x78否定响应
