...
static int isotp_receive_first_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    link->receive_size = payload_length; //本次ISO TP的传输大小
...
static int isotp_receive_consecutive_frame(IsoTpLink *link, IsoTpCanMessage *message, uint8_t len) {
    ...
    /* check data length */
    remaining_bytes = link->receive_size - link->receive_offset;//receive size记录的是最大大小 link->receive_size = payload_length;
    if (remaining_bytes > sizeof(message->as.consecutive_frame.data)) {
        remaining_bytes = sizeof(message->as.consecutive_frame.data);
    }
    if (remaining_bytes > len - 1) { //剩余还比较多，接收就开始减小，这个是错误的
...
int isotp_send_with_id(IsoTpLink *link, uint32_t id, const uint8_t payload[], uint16_t size) {
    ...
    if (link->send_size < 8) {
		...
		//send_bs_remain =  Remaining block size 
		//send_st_min_us = Separation Time between consecutive frames
		//send_timer_st = Last time send consecutive frame
		//send_timer_bs = 两个FC的差值 Time until reception of the next FlowControl N_PDU  start at sending FF, CF, receive FC end at receive FC
        if (ISOTP_RET_OK == ret) {
    ...
    if (ISOTP_SEND_STATUS_INPROGRESS == link->send_status) {
        ...
        (0 == link->send_st_min_us || IsoTpTimeAfter(isotp_user_get_us(), link->send_timer_st))) {
            ...
            if (ISOTP_RET_OK == ret) {
                ...
                link->send_timer_bs = isotp_user_get_us() + ISO_TP_DEFAULT_RESPONSE_TIMEOUT_US; //配置的100ms
                link->send_timer_st = isotp_user_get_us() + link->send_st_min_us;//Last time send consecutive frame
